...
#define E 5					// pin number of uC connected to pin 5 of LCD disp.
...
//modified for shift reg
void LCD_WriteCommand (unsigned char Command) {
	CLR_BIT(CONTROL_BUS,RS);
	//DATA_BUS = Command;
	transmit_data_lower ( Command );
...
void LCD_Cursor(unsigned char column) {
	if ( column < 17 ) { // 16x2 LCD: column < 17; 16x1 LCD: column < 9
		...
		} else { // 16x2 LCD: column - 9; 16x1 LCD: column - 1
